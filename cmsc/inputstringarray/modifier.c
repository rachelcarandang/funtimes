...
/* Lamont: In general it works but you made this program WAAY more 
           complicated then it really should have been. Also you 
           need make sure as you build a program you split it into 
           functions for more modular code. */
...
int main(int argc, char *argv[]) {
	int n=0,m=0, i, j, k, a, integer;
	char x, *word=NULL, **text=NULL, *buffer, *spacesBuffer, 
		*temp, *nullBuffer;
	// Each of the following is read as a ./a.exeseparate string into an array 
	...
	for (i=1; i< argc; i++) {
		...
		if (strcmp(argv[i],"-w")==0) {
			word= argv[i+1]; 
			for (j=0; j<m; j++) {
				// comparing lowercase versions of both
				// command line argument and input text
				// without modifying memory, using temp 
				a=strlen(text[j])+1;
				temp= (char*)malloc(a*sizeof(char));
				temp= memcpy(temp, text[j], a); 
				if (strcmp(lowercase(temp), 
						lowercase(word))==0) {
